import {html} from '../../types/html';

export const render = () => html`
  <div class="stack-large">
    <h1>Admin</h1>
  <p>You have super-user privileges.</p>
  <nav>
    <ul class="stack">
      <li><a href="/members">View all members</a></li>
      <li><a href="/areas">Manage areas and owners</a></li>
      <li><a href="/super-users">Manage super-users</a></li>
      <li>
        <a href="/event-log">View a log of all actions taken</a>
      </li>
      <li>
        <a href="/event-log/failed">View events that failed to apply</a>
      </li>
      <li>
        <a href="/event-log/deleted">View deleted events</a>
      </li>
      <li>
        <a href="/event-log-order">Visualise event log ordering (blocks &amp; seams)</a>
      </li>
      <li>
        <a href="/event-log.csv">Download a log of all actions taken</a>
      </li>
      <li><a href="/members/create">Link an email and number</a></li>
      <li><a href="/training-status.csv">Download current owners and trainers</li>
      <li><a href="/trouble-tickets">View all trouble tickets (prototype)</li>
      <li><a href="/notifications">Manage notification banners</a></li>
      <li><a href="/mailbox">Management mailbox (imported)</a></li>
      <li><a href="/members/rejoined-with-new">Mark member rejoined with new number</a></li>
      <li><a href="/members/rejoined-with-existing">Mark a user as rejoined</a></li>
      <li><a href="/unlinked-recurly">View Recurly emails not linked to members</a></li>
    </ul>
  </nav>
  </div>
`;
