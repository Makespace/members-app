...
import {notImplemented, notImplementedA} from '../not_implemented';
...
export const byEquipmentId: Ord<RegEvent> = pipe(
  S.Ord,
  contramap((e: RegEvent) => e.equipmentId)
);

export const getTrainingSheets = (events: ReadonlyArray<RegEvent>) =>
  pipe(
    events,
...
export const getPreviousQuizResultsByEquipment = accumBy<string, QzEvent>(
  e => e.equipmentId
);

// TODO - Use functional code like the rest of the code base.

const extractGoogleSheetData = (data: GoogleSheetData) => {
  return notImplemented([data]);
};

export const processForEquipment = async (
  logger: Logger,
  deps: Dependencies,
  regEvent: RegEvent,
  ...
  const data = deps.pullGoogleSheetData(logger, regEvent.trainingSheetId);
  const extracted = extractGoogleSheetData(data);
  await notImplementedA([extracted, existingQuizResults]);
