...
const sendPost = (path: string) => (data) => axios.post(
	`${ENDPOINT}/${path}`,
	JSON.stringify(data),
	{
		headers: {
...
const createUser = (user: typeof ADMIN) => sendPost('api/members/create')({
...
const createArea = async (area: typeof WOOD_AREA) => {
	...
	await Promise.all(area.owners.map(owner => sendPost('api/areas/add-owner')({
		areaId: area.id,
...
const createEquipment = (area: typeof WOOD_AREA) => async (equipment: typeof WOOD_LATHE) => {
	...
	await Promise.all(equipment.trainers.map(trainer => sendPost('api/equipment/add-trainer')({
		equipmentId: equipment.id,
		memberNumber: trainer.memberNumber,
	})));
	await Promise.all(equipment.trainedUsers.map(user => sendPost('api/equipment/mark-member-trained')({
		equipmentId: equipment.id,
		memberNumber: user.memberNumber,
	})));
}
